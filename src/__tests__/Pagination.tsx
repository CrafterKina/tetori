...
describe("Pagination機能はArray要素を移動する機能を持つ", () => {
    ...
    test("要素数が現地点より小さい場合にページを終端に合わせる", () => {
        ...
        rerender(<Pagination paginateObjects={[0]} handlePageChange={handlePageChange}/>)

        // Propの変更によるコール
        expect(handlePageChange).not.toHaveBeenLastCalledWith(undefined); // undefinedには決してならない
        expect(handlePageChange).toHaveBeenLastCalledWith(0); // 現在の終端
    })

    test("空配列から要素が追加された場合最初の要素に移動する", () => {
        const handlePageChange = jest.fn()

        const {rerender} = render(<Pagination paginateObjects={[]} handlePageChange={handlePageChange}/>);

        // 初期化コール
        expect(handlePageChange).toHaveBeenLastCalledWith(undefined);
